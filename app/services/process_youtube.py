...
TRANSCRIPT_UNAVAILABLE_MARKER = "_UNAVAILABLE_"
    ...
    for video in videos:
        try:
            ...
            if transcript_result:
                repo.update_youtube_videos_transcript(video.video_id, transcript_result.text)
                processed += 1
            else:
                repo.update_youtube_videos_transcript(video.video_id, TRANSCRIPT_UNAVAILABLE_MARKER)
                unavailable += 1

        except Exception as e:
            failed += 1
            print(f"Error processing video {video.video_id}: {e}")
            # note: video is NOT marked _UNAVAILABLE_ here, so it stays NULL
